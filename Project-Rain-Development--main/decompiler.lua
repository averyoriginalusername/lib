xpcall(function()
	local _, decompiler = pcall(function()
			...
			if (status ~= 0) then
				...
				for i = 1,sizeProtos do
					...
					if (reader:nextByte() == 1) then -- Has Debug info?
						warn'disassemble() can only be called on ROBLOX scripts'
					end
				end
				...
				warn(string.format("Invalid bytecode (version: %i)", status))
				return nil;
			end
		end
		...
		local function disassemble(a1, showLine, showOps)
			if a1:IsA("Script") and not a1:IsA("LocalScript") and not a1:IsA("ModuleScript") then
				return "--[[decompiler cant decompile server you retard]]"
			end
			if (typeof(a1):lower() == "instance") then
				if not getscriptbytecode then warn("Executor does not support getscriptbytecode") end
			...
			local function readProto(proto, depth)
				...
				local englishletters = {'q','w','e','r','t','y','u','i','o','p','a','s','d','f','g','h','j','k','l','z','x','c','v','b','n','m','1','2','3','4','5','6','7','8','9','0'}
				local function validateCanBeString(value)
					local _, v = pcall(function() 
						if type(value) == "string" then
							for i = 1,#(k.value) do
								if not (table.find(englishletters,(string.sub(value,i,i):lower()))) then
									return nil;
								else
									return "." .. value;
								end
				...
				while codeIndex < proto.sizeCode do
					...
					if markedAux then
						...
						if opc == getOpCode("LOADNIL") then
							output = output .. (isVarDefined(A) and "" or "local ") .. string.format("null%i = nil", A)
						...
						elseif opc == getOpCode("GETIMPORT") then
							...
							elseif indexCount == 3 then
								...
								warn("[GETIMPORT] Too many entries");
							end
						...
						elseif opc == getOpCode("GETTABLEKS") then
							local k = proto.kTable[aux + 1] or nilValue;
							local test = validateCanBeString(k.value) or (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value);
							output = output .. (isVarDefined(A) and "" or "local ") .. string.format("v%i = v%i" .. test, A, B)
						elseif opc == getOpCode("SETTABLEKS") then
							local k = proto.kTable[aux + 1] or nilValue;
							local test = validateCanBeString(k.value) or (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value);
							output = output .. string.format("v%i%s = v%i", B, test, A)
						...
						elseif opc == getOpCode("ADDK") then
							local k = proto.kTable[C + 1] or nilValue;
							output = output .. (isVarDefined(A) and "" or "local ") .. string.format("v%i += %s", A, B, tostring(k.value));
						elseif opc == getOpCode("SUB") then
							output = output .. (isVarDefined(A) and "" or "local ") .. string.format("v%i -= v%i", A, B, C);
						...
						elseif opc == getOpCode("MODK") then
							local k = proto.kTable[C + 1] or nilValue;
							output = output .. (isVarDefined(A) and "" or "local ") .. string.format("v%i = v%i", A, B) .. " % " .. tostring(k.value);
							...
							for j = codeIndex + 1, proto.sizeCode do
								if luau.GET_OPCODE(proto.codeTable[j]) ~= getOpCode("CAPTURE") then
									...
									elseif captureType == 2 then
										...
										warn("[NEWCLOSURE] Invalid capture type");
									end
								end
							end
							...
							for j = codeIndex + 1, proto.sizeCode do
								if luau.GET_OPCODE(proto.codeTable[j]) ~= getOpCode("CAPTURE") then
									...
									elseif captureType == 2 then
										...
										warn("[DUPCLOSURE] Invalid capture type");
									end
								end
							end
							codeIndex = codeIndex + nCaptures;
						...
						if not suc then
							warn("warn: " .. err) 
							output = output .. "--[[failed to decompile on line: " .. tostring(codeIndex) .. "]]"
						end
						for _,v in pairs(refData) do
